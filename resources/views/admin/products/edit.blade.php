...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.update", [$product->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.product.fields.variations') }}</label>
                <select class="form-control {{ $errors->has('variations') ? 'is-invalid' : '' }}" name="variations" id="variations" required>
                    <option value disabled {{ old('variations', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Product::VARIATIONS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('variations', $product->variations) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="multi_images">{{ trans('cruds.product.fields.multi_images') }}</label>
                <div class="needsclick dropzone {{ $errors->has('multi_images') ? 'is-invalid' : '' }}" id="multi_images-dropzone">
                </div>
                @if($errors->has('multi_images'))
                    <span class="text-danger">{{ $errors->first('multi_images') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.multi_images_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="profile_id">{{ trans('cruds.product.fields.profile') }}</label>
                <select class="form-control select2 {{ $errors->has('profile') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id">
                    @foreach($profiles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('profile_id') ? old('profile_id') : $product->profile->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="ingredients">{{ trans('cruds.product.fields.ingredients') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.ingredients_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="crusts">{{ trans('cruds.product.fields.crusts') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('crusts') ? 'is-invalid' : '' }}" name="crusts[]" id="crusts" multiple>
                    @foreach($crusts as $id => $crust)
                        <option value="{{ $id }}" {{ (in_array($id, old('crusts', [])) || $product->crusts->contains($id)) ? 'selected' : '' }}>{{ $crust }}</option>
                    @endforeach
                </select>
                @if($errors->has('crusts'))
                    <span class="text-danger">{{ $errors->first('crusts') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.product.fields.crusts_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    var uploadedMultiImagesMap = {}
Dropzone.options.multiImagesDropzone = {
    url: '{{ route('admin.products.storeMedia') }}',
    maxFilesize: 5, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    addRemoveLinks: true,
    ...
    success: function (file, response) {
      $('form').append('<input type="hidden" name="multi_images[]" value="' + response.name + '">')
      uploadedMultiImagesMap[file.name] = response.name
    },
    removedfile: function (file) {
      console.log(file)
      file.previewElement.remove()
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedMultiImagesMap[file.name]
      }
      $('form').find('input[name="multi_images[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($product) && $product->multi_images)
      var files = {!! json_encode($product->multi_images) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview)
          file.previewElement.classList.add('dz-complete')
          $('form').append('<input type="hidden" name="multi_images[]" value="' + file.file_name + '">')
        }
@endif
    },
     error: function (file, response) {
