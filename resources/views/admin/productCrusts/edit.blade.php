...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.productCrust.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-crusts.update", [$productCrust->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id">
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $productCrust->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crusts') ? 'is-invalid' : '' }}" name="crusts[]" id="crusts" multiple>
                    @foreach($crusts as $id => $crust)
                        <option value="{{ $id }}" {{ (in_array($id, old('crusts', [])) || $productCrust->crusts->contains($id)) ? 'selected' : '' }}>{{ $crust }}</option>
                    @endforeach
                </select>
