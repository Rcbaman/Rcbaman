...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.dish.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.dishes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crusts') ? 'is-invalid' : '' }}" name="crusts_id" id="crusts_id">
                    @foreach($crusts as $id => $entry)
                        <option value="{{ $id }}" {{ old('crusts_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="category_id">{{ trans('cruds.dish.fields.category') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.dish.fields.category_helper') }}</span>
            </div>
            <div class="form-group">
