...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.ingredients.update", [$ingredient->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="slug">{{ trans('cruds.ingredient.fields.slug') }}</label>
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $ingredient->slug) }}">
                @if($errors->has('slug'))
                    <span class="text-danger">{{ $errors->first('slug') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.ingredient.fields.slug_helper') }}</span>
            </div>
            <div class="form-group">
