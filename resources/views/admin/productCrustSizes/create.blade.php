...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.productCrustSize.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-crust-sizes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('crust') ? 'is-invalid' : '' }}" name="crust_id" id="crust_id" required>
                    @foreach($crusts as $id => $entry)
                        <option value="{{ $id }}" {{ old('crust_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('crust'))
